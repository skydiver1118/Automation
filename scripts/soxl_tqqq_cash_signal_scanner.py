    ...
    for attempt in range(1, MAX_FETCH_ATTEMPTS + 1):
        try:
            ...
            if attempt < MAX_FETCH_ATTEMPTS:
                print(f"fetch_retry attempt={attempt} reason={exc}")
                time.sleep(FETCH_RETRY_DELAY_SEC * attempt)
    raise RuntimeError(f"Failed to fetch bars after {MAX_FETCH_ATTEMPTS} attempts: {last_exc}") from last_exc
...
def relative_momentum_score(close: pd.DataFrame, config: StrategyConfig) -> pd.DataFrame:
    ...
    vol = daily_returns.rolling(max(lookback, 10)).std() * (252**0.5)
...
def build_base_rotation(close: pd.DataFrame, config: StrategyConfig) -> pd.Series:
    ...
    raw = diff.ge(0).map({True: "SOXL", False: "TQQQ"})
    raw = raw.where(diff.notna(), None)
...
def submit_orders(
    ...
    *,
    ...
    def submit_with_retry(request: LimitOrderRequest | MarketOrderRequest) -> Any:
        last_exc: Exception | None = None
        for attempt in range(1, MAX_ORDER_ATTEMPTS + 1):
            try:
                ...
                if attempt < MAX_ORDER_ATTEMPTS:
                    print(f"order_submit_retry attempt={attempt} symbol={request.symbol} side={request.side} reason={exc}")
                    time.sleep(ORDER_RETRY_DELAY_SEC * attempt)
        raise RuntimeError(f"Failed to submit order for {request.symbol} {request.side}: {last_exc}") from last_exc

    close_by_symbol = {"SOXL": signal.soxl_close, "TQQQ": signal.tqqq_close}
    for symbol, position in positions.items():
        if signal.target == symbol:
            continue
        submitted.append(submit_with_retry(order(symbol, OrderSide.SELL, abs(position.qty), close_by_symbol[symbol])))
    ...
    if signal.target in SYMBOLS and signal.target not in positions:
        ...
        submitted.append(submit_with_retry(order(signal.target, OrderSide.BUY, buy_qty, close_by_symbol[signal.target])))
...
def send_email_summary(
    ...
    *,
    ...
    if not (smtp_host and smtp_user and smtp_password and sender):
        print(
            "\nEmail skipped: missing SMTP environment variables. "
    ...
    try:
        ...
        print(f"\nEmail skipped: invalid ALERT_SMTP_PORT={smtp_port_raw!r}.")
        return
    ...
    if submitted:
        ...
        for order_result in submitted:
            lines.append(
                f"- id={order_result.id} symbol={order_result.symbol} side={order_result.side} "
                f"qty={order_result.qty} limit_price={getattr(order_result, 'limit_price', None)} "
                f"extended_hours={getattr(order_result, 'extended_hours', None)} status={order_result.status}"
            )
    else:
    ...
    print(f"\nemail_sent to={recipient} subject={subject}")
...
def main() -> None:
    ...
    parser.add_argument("--qty", type=float, default=1.0, help="Fallback target shares to buy when --target-notional is 0.")
    parser.add_argument("--target-notional", type=float, default=10000.0, help="Dollar amount to allocate on BUY (e.g., 10000).")
